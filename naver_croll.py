...
class CrawlerApp(QtWidgets.QWidget):
    ...
    async def perform_crawl(self, keyword):
        async with async_playwright() as p:
            ...
            # 상품 정보 크롤링
            products = await page.query_selector_all("CSS_SELECTOR_for_products")  # 적절한 CSS 선택자 사용

            product_data = []
            for product in products:
                name = await product.query_selector("xpath_to_name").inner_text()
                price = await product.query_selector("xpath_to_price").inner_text()
                seller_count = await product.query_selector("xpath_to_seller_count").inner_text()
                options = await product.query_selector("xpath_to_options").inner_text()
                rating = await product.query_selector("xpath_to_rating").inner_text()
                review_count = await product.query_selector("xpath_to_review_count").inner_text()
                date = await product.query_selector("xpath_to_date").inner_text()
                thumbnail_url = await product.query_selector("xpath_to_thumbnail").get_attribute("src")
                shipping_cost = await product.query_selector("xpath_to_shipping_cost").inner_text()
